...
class footballController extends Controller
{
    ...
    public function loadGames()
    {
        ...
        if ($responses == 'error') {
            ...
            <tr>
            <td colspan='8'>
            <p class=' errorMsgBg p-3 white boldSix mt-4 wid60 centerText d-block mx-auto' role='alert'>No data at the moment</p>
            </td>
            </tr>
            ";
        } else {
    ...
    public static function loadLeagueGames($leagueId)
    {
        ...
        if ($responses == 'error') {
            ...
            <tr>
                <td colspan='8'>
                    <p class=' errorMsgBg p-3 white boldSix mt-4 wid80 centerText d-block mx-auto' role='alert'>No data at the moment</p>
                </td>
            </tr>";
        } else {
            $gameUI = "";
    ...
    public static function loadFixtureData($fixtureId)
    {
        ...
        if (!isset(json_decode($response)->response[0])) {
            echo "<p class=' errorMsgBg p-3 white boldSix mt-4 wid80 centerText d-block mx-auto' role='alert'>No data at the moment</p>";
        } else {
                ...
                switch ($response->teams->home->winner) {
                    case null:
                        if (isset($response->teams->home->winner)) {
                            if ($response->teams->home->winner == $teamId) {
                                $winner = "<span class='fontSize12px me-3 padSmall1 borderRA1 white boldFive standingRed'>L</span>";
                            } else {
                                $winner = "<span class='fontSize12px me-3 padSmall1 borderRA1 white boldFive bgBlue'>W</span>";
                            }
                        } else {
                            $winner = "<span class='fontSize12px me-3 padSmall1 borderRA1 white boldFive standingGrey'>D</span>";
                        }
        ...
        if (!isset(json_decode($response)->response[0])) {
            ...
            foreach ($responses as $response) {
                ...
                switch ($response->teams->away->winner) {
                    case null:
                        if ($response->teams->home->winner == false) {
                            $winner = "<span class='fontSize12px me-3 padSmall1 borderRA1 white boldFive standingRed'>L</span>";
                        } else {
                            $winner = "<span class='fontSize12px me-3 padSmall1 borderRA1 white boldFive standingGrey'>D</span>";
                        }
    ...
    public static function countries()
    {
        ...
        if ($responses == 'error') {
            ...
            foreach ($responses->response as $country) {
                echo "
                <a href='$country->code' class='wid100 country bodyA d-block boldFive p-2 marginY fontSize12px bgWhite' onclick='event.preventDefault(); $.ajax({url: \"loadLeagues/" . $country->code . "\" , type: \"GET\", success: (data) => {
                    $(\"#bodyVal\").html(data); }
                })' id='ahref-link-" . $country->code . "' ><img src='" . $country->flag . "'class='wid16px'  alt=''> " . $country->name . "</a>
